            ...
            try
            {
                ...
                if (compileEntry.EnterCount == 1 && _resolversMethod != null)
                {
                    ...
                    if (methodContext != null && methodContext.IsResolved)
                    {
                        ...
                        {
                            //TODO: Need refactor in size of byte-code
                            //Min size byte-code generated by JIT
                            const int minSize = 21;

                            if (methodContext.ByteCode.Length > minSize)
                            {
                                ...
                                //...
                                int nextMax = methodContext.ByteCode.Length + (3 - methodContext.ByteCode.Length % 3);
                ...
                if (methodContext?.Mode == MethodContext.ResolveMode.ASM)
                {
                    Marshal.Copy(methodContext.ByteCode, 0, nativeEntry, methodContext.ByteCode.Length);
                }
